"""
PROGNOZA DESZCZU I SNIEGU - JEDEN SAMODZIELNY PLIK (WERSJA POPRAWIONA)

Program przyjmuje trzy pliki CSV:
1. Wroclaw,
2. Suwalki 2023,
3. Suwalki 2010.

Dla kazdego pliku:
- trenuje lub wczytuje zapisany model,
- testuje model chronologicznie,
- przewiduje opad za 15, 30, 45 i 60 minut,
- rozpoznaje deszcz, snieg albo opad mieszany,
- szacuje ilosc opadu oraz orientacyjna grubosc swiezego sniegu.

Najwazniejsza metoda publiczna:
    RainSnowForecastSystem.forecast_three_csv_files(...)

Zwracany wynik:
    {
        "wroclaw": (flaga, czas_min, szczegoly),
        "suwalki_2023": (flaga, czas_min, szczegoly),
        "suwalki_2010": (flaga, czas_min, szczegoly),
    }
...
Wersja poprawiona automatycznie wykrywa uszkodzone pliki .joblib,
odkłada je jako kopię i wykonuje trening od nowa.
"""
...
class PrecipitationNowcaster:
    ...
    def save(self, path: Path | str) -> None:
        """
        Zapisuje model atomowo. Najpierw powstaje plik tymczasowy, a dopiero
        po poprawnym zapisie zastępuje on właściwy plik. Dzięki temu przerwanie
        programu nie zostawia uszkodzonego pliku nowcaster.joblib.
        """
        path = Path(path)
        ...
        try:
            joblib.dump(self, temporary_path, compress=3)
...
def run_dataset_program(
    ...
    must_train = force_train or not model_path.exists() or not metrics_path.exists()

    if must_train:
        if force_train:
            print(f"\n[INFO] Wymuszono ponowny trening modelu: {dataset_name}")
        else:
            print(f"\n[INFO] Brak kompletnego zapisanego modelu. Rozpoczynam trening: {dataset_name}")

        nowcaster = PrecipitationNowcaster(dataset_name=dataset_name)
        metrics = nowcaster.benchmark_and_fit(raw_df, output_dir)
    else:
        try:
            print(f"\n[INFO] Wczytywanie zapisanego modelu: {model_path}")
            nowcaster = PrecipitationNowcaster.load(model_path)
            metrics = pd.read_csv(metrics_path)
            print(f"[OK] Model został poprawnie wczytany: {dataset_name}")
        except Exception as error:
            # Najczęstszy przypadek: przerwany lub niepełny plik joblib.
            # Nie zatrzymujemy programu — zabezpieczamy wadliwe pliki i uczymy model od nowa.
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            print("\n[OSTRZEZENIE] Nie udało się wczytać zapisanego modelu.")
            print(f"Powód: {type(error).__name__}: {error}")
            print("Uszkodzony lub niezgodny model zostanie odłożony, a program wytrenuje nowy.")

            for damaged_path in (model_path, metrics_path):
                if damaged_path.exists():
                    backup_path = damaged_path.with_name(
                        damaged_path.name + f".uszkodzony_{timestamp}"
                    )
                    try:
                        damaged_path.replace(backup_path)
                        print(f"Zabezpieczono wadliwy plik: {backup_path}")
                    except OSError:
                        damaged_path.unlink(missing_ok=True)
                        print(f"Usunięto wadliwy plik: {damaged_path}")
...
# =====================================================================
# 4. KONFIGURACJA TRZECH ZBIOROW DANYCH
# =====================================================================
DATASETS = {
    "wroclaw": {
        "dataset_name": "wroclaw_2024_2025",
        "default_filename": "wroclaw_pogoda_15min_model.csv",
    },
    "suwalki_2023": {
        "dataset_name": "suwalki_2023",
        "default_filename": "suwalki_pogoda_15_min_model_2023.csv",
    },
    "suwalki_2010": {
        "dataset_name": "suwalki_2010",
        "default_filename": "suwalki_pogoda_15_min_model_2010.csv",
    },
}


def _find_default_csv(filename: str) -> Path:
    """
    Szuka pliku CSV w typowych miejscach repozytorium.

    Zakladana lokalizacja programu:
        Benchmark/benchmark/Prognoza_deszczu_i_sniegu/

    Obslugiwane lokalizacje CSV:
    - obok programu,
    - w podfolderze data,
    - w Benchmark/benchmark/Pogoda_pomiary_15_minut,
    - w aktualnym katalogu roboczym PyCharma.
    """
    ...
    candidates = [
        ...
        benchmark_dir / "Pogoda_pomiary_15_minut" / filename,
        benchmark_dir / filename,
        Path.cwd() / filename,
        Path.cwd() / "data" / filename,
        Path.cwd() / "Pogoda_pomiary_15_minut" / filename,
    ]
    ...
    raise FileNotFoundError(
        ...
        f"Podaj sciezke argumentem programu albo umiesc CSV w jednej z lokalizacji:\n"
        f"{checked}"
    )


class RainSnowForecastSystem:
    """
    Klasa integrujaca prognozowanie dla wszystkich trzech plikow CSV.

    Publiczna metoda forecast_three_csv_files() jest glownym punktem
    korzystania z calego modulu.
    """

    def __init__(self, output_root: Path | str = "wyniki_prognozy"):
        self.output_root = Path(output_root)

    def forecast_three_csv_files(
        self,
        wroclaw_csv: Path | str,
        suwalki_2023_csv: Path | str,
        suwalki_2010_csv: Path | str,
        force_train: bool = False,
    ) -> Dict[str, Tuple[bool, Optional[int], Dict[str, Any]]]:
        """
        Trenuje/wczytuje modele i wykonuje prognoze dla trzech CSV.

        PARAMETRY
        ---------
        wroclaw_csv:
            Sciezka do pliku wroclaw_pogoda_15min_model.csv.
        suwalki_2023_csv:
            Sciezka do pliku suwalki_pogoda_15_min_model_2023.csv.
        suwalki_2010_csv:
            Sciezka do pliku suwalki_pogoda_15_min_model_2010.csv.
        force_train:
            True - wymusza ponowny trening.
            False - korzysta z zapisanych modeli, jesli istnieja.

        ZWRACA
        ------
        Slownik zawierajacy trzy wyniki. Kazdy wynik jest krotka:

            (flaga, czas_min, szczegoly)

        flaga:
            True - model przewiduje opad w ciagu 60 minut.
            False - model nie przewiduje opadu.
        czas_min:
            Najwczesniejszy horyzont 15, 30, 45 albo 60 minut.
            None, jezeli opad nie jest przewidywany.
        szczegoly:
            Typ opadu, prawdopodobienstwo, ilosc opadu, ilosc sniegu
            i wyniki dla wszystkich czterech horyzontow.
        """
        csv_paths = {
            "wroclaw": Path(wroclaw_csv),
            "suwalki_2023": Path(suwalki_2023_csv),
            "suwalki_2010": Path(suwalki_2010_csv),
        }

        results: Dict[str, Tuple[bool, Optional[int], Dict[str, Any]]] = {}
        ...
        for key, csv_path in csv_paths.items():
            config = DATASETS[key]
            dataset_output = self.output_root / key
            ...
            metrics["source_key"] = key
            all_metrics.append(metrics)
            results[key] = result

        combined_metrics = pd.concat(all_metrics, ignore_index=True)
        combined_metrics.to_csv(
            self.output_root / "benchmark_wszystkie_3_pliki.csv",
            index=False,
        )

        serializable_results = {
            key: {
                "will_precipitate": value[0],
                "lead_time_min": value[1],
                "details": value[2],
            }
            for key, value in results.items()
        }

        with (self.output_root / "prognoza_wszystkie_3_pliki.json").open(
            "w", encoding="utf-8"
        ) as handle:
            json.dump(serializable_results, handle, ensure_ascii=False, indent=2)
...
def _print_single_result(
    dataset_label: str,
    result: Tuple[bool, Optional[int], Dict[str, Any]],
) -> None:
    ...
    print(dataset_label)
    print("=" * 84)
    print(f"Flaga opadu: {flag}")
    print(f"Najwczesniejszy czas: {lead_time_min} min")
    print(f"Typ opadu: {details.get('type', 'none')}")
    print(f"Prawdopodobienstwo: {details.get('probability', 0.0) * 100:.2f}%")
    print(
        "Ilosc opadu: "
        f"{details.get('precipitation_amount_mm_15min', 0.0):.4f} mm / 15 min"
    )
    print(
        "Ekwiwalent wodny sniegu: "
        f"{details.get('snow_water_equivalent_mm', 0.0):.4f} mm"
    )
    print(
        "Szacowana grubosc swiezego sniegu: "
        f"{details.get('estimated_fresh_snow_depth_cm', 0.0):.4f} cm"
    )

    print("\nWszystkie horyzonty:")
    print(
        f"{'Czas':>7} | {'Opad':>6} | {'Prawd.':>9} | "
        f"{'Typ':>7} | {'mm/15 min':>10} | {'snieg cm':>9}"
    )
    print("-" * 67)

    for horizon in details.get("horizons", []):
        print(
            f"{horizon['horizon_min']:>5} m | "
            f"{str(horizon['will_precipitate']):>6} | "
            f"{horizon['probability'] * 100:>8.2f}% | "
            f"{horizon['precipitation_type']:>7} | "
            f"{horizon['precipitation_amount_mm_15min']:>10.4f} | "
            f"{horizon['estimated_fresh_snow_depth_cm']:>9.4f}"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Jeden program prognozujacy deszcz i snieg na podstawie "
            "trzech plikow CSV."
        )
    )
    parser.add_argument(
        "--wroclaw",
        default=None,
        help="Sciezka do pliku CSV dla Wroclawia.",
    )
    parser.add_argument(
        "--suwalki-2023",
        dest="suwalki_2023",
        default=None,
        help="Sciezka do pliku CSV dla Suwalk 2023.",
    )
    parser.add_argument(
        "--suwalki-2010",
        dest="suwalki_2010",
        default=None,
        help="Sciezka do pliku CSV dla Suwalk 2010.",
    )
    parser.add_argument(
        "--force-train",
        action="store_true",
        help="Wymus ponowny trening wszystkich modeli.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Opcjonalny folder wynikow.",
    )
    ...
    output_root = (
        Path(args.output)
        if args.output
        else script_dir / "wyniki_prognozy"
    )

    wroclaw_path = (
        Path(args.wroclaw)
        if args.wroclaw
        else _find_default_csv(DATASETS["wroclaw"]["default_filename"])
    )
    suwalki_2023_path = (
        Path(args.suwalki_2023)
        if args.suwalki_2023
        else _find_default_csv(DATASETS["suwalki_2023"]["default_filename"])
    )
    suwalki_2010_path = (
        Path(args.suwalki_2010)
        if args.suwalki_2010
        else _find_default_csv(DATASETS["suwalki_2010"]["default_filename"])
    )

    print("=" * 84)
    print("PROGNOZA DESZCZU I SNIEGU - 3 PLIKI CSV")
    print("=" * 84)
    print(f"Wroclaw:      {wroclaw_path}")
    print(f"Suwalki 2023: {suwalki_2023_path}")
    print(f"Suwalki 2010: {suwalki_2010_path}")
    print(f"Folder wynikow: {output_root}")
    print(f"LightGBM dostepny: {LIGHTGBM_AVAILABLE}")

    system = RainSnowForecastSystem(output_root=output_root)

    results = system.forecast_three_csv_files(
        wroclaw_csv=wroclaw_path,
        suwalki_2023_csv=suwalki_2023_path,
        suwalki_2010_csv=suwalki_2010_path,
        force_train=args.force_train,
    )

    labels = {
        "wroclaw": "WROCLAW",
        "suwalki_2023": "SUWALKI 2023",
        "suwalki_2010": "SUWALKI 2010",
    }

    for key, result in results.items():
        _print_single_result(labels[key], result)

    print("\n" + "=" * 84)
    print("GLOWNY ZWROCONY WYNIK")
    print("=" * 84)
    print(results)
    print(f"\nZapisano wyniki w folderze: {output_root}")
